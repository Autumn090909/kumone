...
extension Audition {
    ...
    public struct ExchangeReport: Encodable, Sendable {
        ...
        /// Seconds the two voices take to change places at L — at most one
        /// beat. The outgoing vocal is at full level right up to L and gone
        /// this long after it; nothing about either vocal lane is gradual
        /// outside this window.
        public let handoverCrossSeconds: TimeInterval
        /// The incoming vocal's own clock: seconds into the overlap where deck
        /// B's singer is let in. Equal to `handover` on the single-clock shape.
        public let incomingRelease: TimeInterval
        /// The incoming line the new singer enters on, when the second clock
        /// had one to offer.
        public let incomingLine: String?
        /// `"pair"` / `"nextLine"` / `"handover"` — how the two instants were
        /// chosen. See `Audition.ExchangeCompilation.pairSource`.
        public let pairSource: String
    }
        ...
        return DecisionReport(
            ...
            stemExchange: d.stemExchange.map {
                ExchangeReport(handover: $0.handover,
                               ...
                               handoverCrossSeconds: $0.handoverCrossSeconds,
                               incomingRelease: $0.incomingRelease,
                               incomingLine: $0.incomingLine,
                               pairSource: $0.pairSource)
            },
            stemEnvelope: d.stemEnvelope.map { envelope in
                EnvelopeReport(
                    overlap: d.overlapDuration,
                    minGainDB: Double(StemEnvelope.minGainDB),
                    // The plot's ceiling, not the contract's: a compensated
                    // lane legitimately goes past `maxGainDB`, and a graph that
                    // silently drops those points is a graph that hides exactly
                    // the part of the gesture worth looking at.
                    maxGainDB: Double(max(StemEnvelope.maxGainDB, envelope.peakGainDB)),
    ...
    private static func stemStep(
        ...
        if let x = d.stemExchange {
            if let reason = x.fallbackReason {
                ...
                // The second clock: when deck B's singer is let in, and on
                // which line. Without it the report describes half the gesture.
                outcome += String(format: "　入曲人声在第 %.2f 秒进入", x.incomingRelease)
                switch x.pairSource {
                case "pair":
                    outcome += "，正好接上入曲歌词「\(x.incomingLine ?? "")」的起句"
                        + "（两个歌词时钟对上了）。"
                case "nextLine":
                    outcome += "——没有能与出曲行末配对的起句，"
                        + "于是等到入曲歌词「\(x.incomingLine ?? "")」开口才放，"
                        + "以免从句子中间切进来。"
                default:
                    outcome += "，入曲这段没有歌词起句可等，和交接句同一时刻放开。"
                }
                // The shape, not just the instant: "where" used to be the whole
                // story because the voice faded either side of it. It no longer
                // does, and a report that did not say so would be describing
                // last month's gesture.
                outcome += String(
                    format: "　出曲人声在这之前一直是满电平（不渐弱），到点之后 %.0f 毫秒内"
                        + "与入曲人声等功率交接完毕——渐变只留给伴奏。",
                    x.handoverCrossSeconds * 1000)
