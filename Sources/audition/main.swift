...
usage:
  ...
  audition render <fileA> <fileB> [-o out.wav] [--style plain|sweep|echo|staged] [--fade N]
                                  ...
                                  [--pre N] [--post N] [--ride-release N] [--master-limiter]
                                  [--set name=value,...]
  ...
  --ride-release  dB/s the gain ride is let go of at, overriding the shipped
            ...
            out of one binary.
  --master-limiter
            render the deck levels of the master-limiter regime
            (`AutoMixOverrides.enableMasterLimiter`): no bent-rate headroom
            pad, so the outgoing deck is not ducked for the fifteen to
            twenty-five seconds before the seam. Render only, and the second
            knob that is not a `--set` name — the limiter itself is a node on
            the player's master path and cannot be written into a file, so
            what this A/Bs is the ducking the pad used to cost.
...
func runRender(_ args: Arguments) {
    ...
    do {
        let r = try Audition.render(d, to: out, preRoll: opts.pre, postRoll: opts.post,
                                    ...
                                    masterLimiter: args.flags["master-limiter"] != nil,
                                    stemProvider: StemService.shared.provider,
                                    fullStemProvider: StemService.shared.fullProvider)
        var stemLine = ""
        ...
        if let x = d.stemExchange {
            if let reason = x.fallbackReason {
                ...
                stemLine += "\n    hand-over at +\(f(x.handover, 2))s into the overlap"
                    ...
                    + (x.lyricLine.map { ": “\($0)”" } ?? "")
                // The other clock: where deck B's voice is actually let in,
                // and on which of *its* lines.
                stemLine += "\n    incoming vocal in at +\(f(x.incomingRelease, 2))s"
                    + " (\(x.pairSource))"
                    + (x.incomingLine.map { ": “\($0)”" } ?? "")
                // The gesture is a plateau and an event, not a curve — so the
                // one number that says how abrupt it is belongs on this line.
                stemLine += "\n    outgoing vocal held at full level to there,"
                    + " then an equal-power exchange over"
                    + " \(f(x.handoverCrossSeconds * 1000, 0))ms (no vocal fade)"
