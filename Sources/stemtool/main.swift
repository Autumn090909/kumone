...
let usage = """
    ...
    OPTIONS:
      ...
      --model <name>       vocals (default, 2 stems) or 4stem
                           (BS-RoFormer drums/bass/other/vocals; needs
                           Scripts/fetch-4stem-checkpoint.sh to have been run)
    ...
    OUTPUT:
      ...
      <dir>/drums.wav …         one file per lane, under --model 4stem
      <dir>/residual.wav        mixture - the four lanes, under --model 4stem
...
var repeatCount = 1
var descriptor = ModelDescriptor.zfturboVocalsV1
...
while index < arguments.count {
    ...
    case "--model":
        switch nextValue("--model", &index) {
        case "vocals": descriptor = .zfturboVocalsV1
        case "4stem": descriptor = .bsRoformerFourStem
        case let other: fail("unknown --model '\(other)'. Expected vocals or 4stem.")
        }
    case "--repeat":
...
Task {
    ...
    do {
        ...
        print("model      \(descriptor.fileName) "
            + "(\(descriptor.configuration.stemOrder.map(\.rawValue).joined(separator: "/")))")
        let separator = try await StemSeparator.prepare(
            modelStore: store, descriptor: descriptor) { fraction in
        ...
        for lane in descriptor.configuration.stemOrder {
            guard let channels = stems.lanes[lane] else { continue }
            try writeWAV(channels, to: outputURL.appendingPathComponent("\(lane.rawValue).wav"),
        ...
        try writeWAV(mixture, to: outputURL.appendingPathComponent("mixture.wav"),
            sampleRate: sampleRate)
        if descriptor.configuration.numStems > 1 {
            try writeWAV(stems.residual, to: outputURL.appendingPathComponent("residual.wav"),
                sampleRate: sampleRate)
        }
        ...
        for lane in descriptor.configuration.stemOrder {
            guard let channels = stems.lanes[lane] else { continue }
            print(
                "\(lane.rawValue.padding(toLength: 10, withPad: " ", startingAt: 0))"
                    + " RMS \(decibels(rms(channels)))  peak \(decibels(peak(channels)))  "
                    + "ratio \(String(format: "%.3f", rms(channels) / max(mixtureRMS, 1e-12)))")
        }
        print(
            "accomp.    RMS \(decibels(rms(stems.accompaniment)))  peak "
                + "\(decibels(peak(stems.accompaniment)))")
        if descriptor.configuration.numStems > 1 {
            // How much of the mixture the four estimated masks fail to account
            // for. This is the number that decides whether "other" may be
            // derived as a residual downstream — see `SeparatedStems.partition`.
            print(
                "residual   RMS \(decibels(rms(stems.residual)))  peak "
                    + "\(decibels(peak(stems.residual)))  "
                    + "ratio \(String(format: "%.3f", rms(stems.residual) / max(mixtureRMS, 1e-12)))")
        }
        print(
